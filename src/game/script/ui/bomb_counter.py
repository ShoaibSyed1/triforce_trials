...
class BombCounter(Script):
    ...
    def update(self, dt):
        for event in self.event_bus.get_events():
            if event.ty == EventType.PLAYER:
                if event.data['type'] == PlayerEventType.BOMBS_CHANGED:
    ...
    def update_text(self, new):
        ...
        self.text_entity = self.world.create_entity_with(*loader.load("entity", "ui/overworld/bomb_text")[0])
        
        transform = self.world.component_for_entity(self.text_entity, Transform)

        transform.position.x = self.transform.position.x + 24
